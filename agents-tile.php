...
if (isset($FirmID)) {
   ...
   $str_url_vars .= "firm_id=$FirmID";
   $str_url_var_num++;
}
if (isset($OfficeID)) {
   if ($str_url_var_num > 0) { $str_url_vars .= "&"; }
   $str_url_vars .= "office_id=$OfficeID";
   $str_url_var_num++;
}
/* Construct Where clause */
if (!empty($FirmID)) {
   $where_clause .= " $db_firm_id = '".$FirmID."'";
   $where_clause_num++;
   if (isset($OfficeID)) {
      $where_clause .= " AND $db_office_id = '".$OfficeID."'";
      $where_clause_num++;
   }
}
if (!empty($AgentID)) {
   $where_clause .= " AND $db_agent_id = '".$AgentID."'";
   $where_clause_num++;
}
