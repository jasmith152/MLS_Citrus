...
<? }elseif($dbtosearch == "marion_mls"){ 
	$sql = "SELECT * FROM tbl_idx_agent WHERE TRIM(AgentID) = '$agent_id'";
	//Include Db connection script
	//include '../../marion/dbconn.php';
	// run the query
	try {
   		$results = $dbcnx->prepare($sql);
		$results->execute();
...
<table align="left" width="40%" border="0" cellspacing="3" cellpadding="3" style='border: 1px solid #000;'>
<tr>
    <td align="center" colspan="2">Agent Details from the marion_mls Db</td>
  </tr>
  <tr>
    <td width="36%">AgentCity</td>
    <td width="64%"><? echo $row['AgentCity'] ?></td>
...
$sql = "SELECT * FROM tbl_agents_info WHERE TRIM(agent_id) = '$agent_id'";
	...
	try {
   		$results = $dbcnx->prepare($sql);
		$results->execute();
		$row_ncwd = $results->fetch(PDO::FETCH_ASSOC);
	} catch(PDOException $e) {
  		echo "Error running query: ".$e->getMessage()."<br />\n";
	}
?>
...
<table align="right" width="40%" border="0" cellspacing="3" cellpadding="3" style='border: 1px solid #000;'>
  <tr>
   ...
   <tr>
    <td width="35%">agent_id</td>
    <td width="65%"><input type='text' name='agent_id' size='20' value='<? echo $row_ncwd['agent_id'] ?>' /></td>
  </tr>
  <tr>
    <td>firm_id</td>
    <td><input type='text' name='firm_id' size='20' value='<? echo $row_ncwd['firm_id'] ?>' /></td>
  </tr>
  <tr>
    <td>off_id</td>
    <td><input type='text' name='off_id' size='20' value='<? echo $row_ncwd['off_id'] ?>' /></td>
  </tr>
  <tr>
    <td>title</td>
    <td><input type='text' name='title' size='20' value='<? echo $row_ncwd['title'] ?>' /></td>
  </tr>
  <tr>
    <td>fname</td>
    <td><input type='text' name='fname' size='20' value='<? echo $row_ncwd['fname'] ?>' /></td>
  </tr>
  <tr>
    <td>lname</td>
    <td><input type='text' name='lname' size='20' value='<? echo $row_ncwd['lname'] ?>' /></td>
  </tr>
  <tr>
    <td>phone1</td>
    <td><input type='text' name='phone1' size='20' value='<? echo $row_ncwd['phone1'] ?>' /></td>
  </tr>
  <tr>
    <td>phone2</td>
    <td><input type='text' name='phone2' size='20' value='<? echo $row_ncwd['phone2'] ?>' /></td>
  </tr>
  <tr>
    <td>fax</td>
    <td><input type='text' name='fax' size='20' value='<? echo $row_ncwd['fax'] ?>' /></td>
  </tr>
  <tr>
    <td>email_leads</td>
    <td><input type='text' name='email_leads' size='20' value='<? echo $row_ncwd['email_leads'] ?>' /></td>
  </tr>
  <tr>
    <td>email_reports</td>
    <td><input type='text' name='email_reports' size='20' value='<? echo $row_ncwd['email_reports'] ?>' /></td>
  </tr>
  <tr>
    <td>descr</td>
    <td><input type='text' name='descr' size='20' value='<? echo $row_ncwd['descr'] ?>' /></td>
  </tr>
  <tr>
    <td>website1</td>
    <td><input type='text' name='website1' size='20' value='<? echo $row_ncwd['website1'] ?>' /></td>
  </tr>
  <tr>
    <td>website2</td>
    <td><input type='text' name='website2' size='20' value='<? echo $row_ncwd['website2'] ?>' /></td>
  </tr>
  <tr>
    <td>service_level</td>
    <td><input type='text' name='service_level' size='20' value='<? echo $row_ncwd['service_level'] ?>' /></td>
  </tr>
  <tr>
    <td>sort_num</td>
    <td><input type='text' name='sort_num' size='20' value='<? echo $row_ncwd['sort_num'] ?>' /></td>
  </tr>
  <tr>
    <td>credentials</td>
    <td><input type='text' name='credentials' size='20' value='<? echo $row_ncwd['credentials'] ?>' /></td>
  </tr>
  <tr>
    <td align="center" colspan="2"><?php if (file_exists($mugs.'agent'.$row_ncwd['agent_id'].'.jpg')) {
            echo "<img src='".$mugs."agent".$row_ncwd['agent_id'].".jpg' alt='agent photo on file' border='0' />";
         } ?></td>
  </tr>
  <tr>
    <td colspan="2" align="center"><input name="submit_update" type="submit" value="submit">
    <input name="dbtosearch" type="hidden" value="<?php echo $dbtosearch; ?>"></td>
  </tr>
</table>
</form>
